...
export default function SeeAllPage() {
  ...
  const [tmdbMin, setTmdbMin] = useState(Number(searchParams.get("tmdbMin") || 0));
  const [pickedProviders, setPickedProviders] = useState(() => {
    const qp = (searchParams.get("provider") || "").toLowerCase();
    return qp ? qp.split(",").map((s) => s.trim()).filter(Boolean) : [];
  });
  const [includeRentBuy, setIncludeRentBuy] = useState(searchParams.get("includeRentBuy") === "1");
  ...
  const buildQuery = useCallback(
    (nextPage) => {
      ...
      if (providersParam) params.providers = providersParam;
  ...
  // Selected filters summary (chips) - KR 19/09/2025
  const chips = useMemo(() => {
    const c = [];
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(1000px 400px at 20% -200px, rgba(111,66,193,.25), rgba(111,66,193,0) 60%), radial-gradient(800px 300px at 90% -120px, rgba(13,110,253,.25), rgba(13,110,253,0) 60%)",
      }}
    >
      <div className="container">
        {/* Header - KR 19/09/2025 */}
        <div className="d-flex align-items-center justify-content-between mb-3">
          <div>
            <h1 className="h3 m-0 text-dark text-capitalize">
          ...
          {/* Right-side actions KR 19/09/2025 */}
          <div className="d-flex align-items-center gap-2">
            <Link className="btn btn-outline-light btn-sm" to="/">
              ← Home
            </Link>
            {/* Mobile - open Filters KR 19/09/2025 */}
            <button
        ...
        <div className="card bg-dark border-0 shadow-sm mb-4 d-none d-md-block">
          <div className="card-body">
            <div className="row g-3 align-items-end">
              ...
              {/* Include rent/buy KR 19/09/2025 */}
              <div className="col-md-2">
                ...
                <div className="form-check form-switch">
                  <input
                    ...
                    type="checkbox"
                    id="includeRentBuySwitch"
                    checked={includeRentBuy}
                    onChange={(e) => setIncludeRentBuy(e.target.checked)}
                    disabled={loading}
                  />
                  <label className="form-check-label small text-white" htmlFor="includeRentBuySwitch">
            ...
            {/* Actions inside the filter container KR 19/09/2025 */}
        ...
        {loadingInitial ? (
          ...
          <>
            ...
            {/* Load more + sentinel for infinite scroll  KR 19/09/2025 */}
      ...
      {/* MOBILE FILTERS: Off-canvas sheet for responsiveness KR 19/09/2025 */}
      <div
      ...
      >
        ...
        <div className="offcanvas-body">
          ...
          {/* Include buy/rent KR 19/09/2025 */}
          <div className="mt-3">
            <div className="form-check form-switch">
              <input
                className="form-check-input"
                type="checkbox"
                id="mobileIncludeRentBuy"
                checked={includeRentBuy}
                onChange={(e) => setIncludeRentBuy(e.target.checked)}
                disabled={loading}
              />
              <label className="form-check-label" htmlFor="mobileIncludeRentBuy">
                Include buy/rent results
              </label>
            </div>
          </div>

          {/* Actions KR 19/09/2025 */}
