...
def generate_and_store_code(user):
    """Generate a 6-digit code, store in cache with TTL; rate-limit sends."""
    rk = _rate_key(user.id)
    if cache.get(rk):
        # Still within rate window; don't re-send email. KR 02/10/2025
        return None
